...
<div class="container-fluid">
    <div class="page-header">
        <div class="row">
            <div class="col-lg-6">
                <h3>Hasil Assesmen By Job Family</h3>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item">Assesmen</li>
                    <li class="breadcrumb-item active">Hasil Assesmen By Job Family</li>
                </ol>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="mb-3 row">
                    <div class="mb-3 row">
                        <label class="col-sm-3 col-form-label">Kode Job Family</label>
                        <div class="col-sm-9">
                            <input class="form-control kode" type="text" name="kode_detail"
                                id="kode_detail" value="" readonly />
                        </div>
                    </div>
...
<script>
    $(document).ready(function () {
        ...
        $(document).on("click", ".delete-btn", function () {
            ...
            id_hasil_del = item.job_family_id;
        });

        $(document).on("click", ".detail-btn", function () {
            var data = table.row($(this).closest('tr')).data();
            console.log(data.job_family.kode);
            document.getElementById("rekomendasi").innerHTML = "Detail "+data.user.nama
            $('#kode_detail').val(data.job_family.kode)
            $('#nama_detail').val(data.job_family.job_family)
            var html = '';
            $.ajax({
                type: "GET",
                url: '/hasil/show/job_family/' + data.job_family_id,
                cache: false,
                success: function (res) {
                    ...
                    $.each(res.data, function (key, item) {
                        html_edit = '<tr>'
                        html_edit += '<td>'+i+'</td>'
                        html_edit += '<td>'+item.user.no_pegawai+'</td>'
                        html_edit += '<td>'+item.user.nama+'</td>'
        ...
        $(document).on("click", "#konfirmasi-del", function () {
            ...
            $.ajax({
                type: "DELETE",
                url: '/hasil/destroy/job_family/' + id_hasil_del,
                cache: false,
                success: function (res) {
                    if (res.status == 200) {
